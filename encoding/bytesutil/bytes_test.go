...
import (
	...
	"github.com/ethereum/go-ethereum/encoding/bytesutil"
...
func TestSetBit(t *testing.T) {
	...
	for _, tt := range tests {
		assert.Equal(t, tt.c, bytesutil.SetBit(tt.a, tt.b))
	}
}
...
func TestClearBit(t *testing.T) {
	...
	for _, tt := range tests {
		assert.Equal(t, tt.c, bytesutil.ClearBit(tt.a, tt.b))
	}
}
...
func TestMakeEmptyBitfields(t *testing.T) {
	...
	for _, tt := range tests {
		assert.Equal(t, tt.b, len(bytesutil.MakeEmptyBitlists(tt.a)))
	}
}
...
func TestHighestBitIndex(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if !tt.error {
			...
			assert.Equal(t, tt.b, i)
		} else {
			assert.Contains(t, err.Error(), "input list can't be empty or nil")
		}
	}
}
...
func TestBytesInvalidInputs(t *testing.T) {
	...
	assert.Equal(t, []byte{}, rawBytes)

	_, err := bytesutil.HighestBitIndexAt([]byte{'A', 'B', 'C'}, -5)
	assert.Contains(t, err.Error(), "index is negative")
...
func TestIsValidRoot(t *testing.T) {
	...
	zeroRoot := make([]byte, RootLength)

	validRoot := make([]byte, RootLength)
	validRoot[0] = 'a'

	wrongLengthRoot := make([]byte, RootLength-4)
	wrongLengthRoot[0] = 'a'
